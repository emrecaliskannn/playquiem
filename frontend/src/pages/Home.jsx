...
function AdSlot() {
  const Box = ({ h, label, size }) => (
    <div style={{
      borderRadius: 12, border: '1px dashed rgba(102,192,244,0.16)',
      background: 'rgba(102,192,244,0.02)',
      ...
      <div style={{
        ...
        color: 'rgba(102,192,244,0.25)', fontSize: 11,
      }}></div>
      <span style={{
        fontFamily: FONT, fontSize: '0.52rem', fontWeight: 800,
        letterSpacing: '0.16em', textTransform: 'uppercase',
        color: 'rgba(102,192,244,0.2)', textAlign: 'center',
      }}>{label}</span>
      {size && (
        <span style={{ fontFamily: FONT, fontSize: '0.48rem',
                        color: 'rgba(102,192,244,0.12)', textAlign: 'center' }}>
          {size}
        </span>
      )}
    </div>
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:14, margin:'8px 0 32px' }}>
      <div style={{ flex:1, height:1,
        background:'linear-gradient(to right, rgba(102,192,244,0.3), rgba(102,192,244,0.04))' }}/>
      {label && (
        <span style={{ fontFamily:FONT, fontSize:'0.55rem', fontWeight:800,
                        letterSpacing:'0.18em', textTransform:'uppercase',
                        color:'rgba(102,192,244,0.3)', whiteSpace:'nowrap' }}>
          {label}
        </span>
      )}
      <div style={{ flex:1, height:1,
        background:'linear-gradient(to left, rgba(102,192,244,0.3), rgba(102,192,244,0.04))' }}/>
    </div>
  )
}
      ...
      <div style={{ display:'flex', alignItems:'center', flexWrap:'wrap', gap:10,
        ...
        {badge && (
          <span style={{ fontFamily:FONT, fontSize:'0.58rem', fontWeight:800,
                          ...
                          background:'rgba(102,192,244,0.1)', border:'1px solid rgba(102,192,244,0.28)',
        ...
        {sub && (
          <span style={{ fontFamily:FONT, fontSize:'0.6rem',
                          color:'rgba(102,192,244,0.25)', letterSpacing:'0.04em',
...
function HScrollRow({ children }) {
  ...
  const Arrow = ({ dir }) => {
    ...
    return (
      <button onClick={() => scroll(dir)} disabled={!enabled} style={{
        ...
        border: `1px solid ${enabled ? 'rgba(102,192,244,0.35)' : 'rgba(102,192,244,0.1)'}`,
        color: enabled ? TEAL : 'rgba(102,192,244,0.2)',
      ...
      onMouseEnter={e => { if (enabled) e.currentTarget.style.background='rgba(102,192,244,0.15)' }}
  ...
  return (
    <div style={{ position:'relative', margin:'0 20px', marginBottom:0 }}>
      ...
      <Arrow dir={1} />
      {canLeft  && <div style={{ position:'absolute', top:0, left:0, bottom:8, width:40, pointerEvents:'none', background:'linear-gradient(to right, #09090f, transparent)' }}/>}
      {canRight && <div style={{ position:'absolute', top:0, right:0, bottom:8, width:40, pointerEvents:'none', background:'linear-gradient(to left, #09090f, transparent)' }}/>}
    </div>
  )
}
...
function PopCard({ game, rank }) {
  ...
  const isTop3    = rank <= 3
  const border = isTop3 ? 'rgba(102,192,244,0.45)' : isSleeper ? 'rgba(179,157,219,0.35)' : game.steamTrending ? 'rgba(102,192,244,0.25)' : 'rgba(255,255,255,0.05)'
  return (
    <div onClick={() => nav('/game/' + game.id)}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        width:138, borderRadius:14, overflow:'hidden', cursor:'pointer', flexShrink:0,
        border:`1px solid ${hov ? 'rgba(102,192,244,0.55)' : border}`,
        transform: hov ? 'translateY(-6px)' : 'translateY(0)',
        boxShadow: hov ? '0 16px 40px rgba(102,192,244,0.12)' : 'none',
      ...
      <div style={{ position:'absolute', top:7, left:8, fontFamily:FONT,
                    ...
                    textShadow:'0 2px 20px rgba(102,192,244,0.7)' }}>#{rank}</div>
      {game.badge && (
        <div style={{
          position:'absolute', top:7, right:6,
          background: isSleeper ? 'rgba(179,157,219,0.2)' : 'rgba(102,192,244,0.12)',
          border: isSleeper ? '1px solid rgba(179,157,219,0.4)' : '1px solid rgba(102,192,244,0.35)',
          color: isSleeper ? 'var(--accent2)' : TEAL,
      ...
      <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'32px 10px 10px',
        ...
        {game.ccuLabel && (
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontFamily:FONT, fontSize:'0.58rem', color:'rgba(102,192,244,0.55)',
                          display:'flex', alignItems:'center', gap:3 }}>
              <span>👥</span>
              <span style={{ fontWeight:700 }}>{game.ccuLabel}</span>
              <span style={{ opacity:0.6 }}>avg players</span>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div onClick={() => game.igdb_id && nav('/game/' + game.igdb_id)}
      style={{ cursor:'pointer', marginBottom:36, borderRadius:16, overflow:'hidden',
               border:'1px solid rgba(102,192,244,0.15)', background:'var(--bg)', position:'relative' }}
      onMouseEnter={e => e.currentTarget.style.borderColor='rgba(102,192,244,0.3)'}
      onMouseLeave={e => e.currentTarget.style.borderColor='rgba(102,192,244,0.15)'}>
      {game.cover_url && (
      ...
      <div style={{ position:'relative', display:'flex', gap:16, padding:'clamp(16px,4vw,28px) clamp(16px,4vw,32px)', alignItems:'center' }}>
        {game.cover_url && (
          <img src={game.cover_url} alt={game.title} style={{ width:'clamp(60px,15vw,100px)', borderRadius:10, flexShrink:0,
            boxShadow:'0 8px 40px rgba(0,0,0,0.8), 0 0 0 1px rgba(102,192,244,0.15)' }}
            onError={e => e.target.style.display='none'} />
        )}
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontFamily:FONT, fontSize:'0.55rem', fontWeight:800, letterSpacing:'0.2em',
                        textTransform:'uppercase', color:'rgba(102,192,244,0.4)', marginBottom:8 }}>
            Recently Finished
          </div>
          <div style={{ fontFamily:FONT, fontWeight:900, fontSize:'clamp(1.2rem,4vw,1.7rem)', letterSpacing:'-0.03em',
                        color:TEAL, lineHeight:1.1, marginBottom:8 }}>{game.title}</div>
          {game.rating > 0 && (
            <div style={{ display:'flex', gap:2, marginBottom:10 }}>
              {[1,2,3,4,5].map(n => (
                <span key={n} style={{ fontSize:'1rem', color: n <= game.rating ? TEAL : 'rgba(102,192,244,0.15)' }}>★</span>
              ))}
            </div>
          )}
          {game.review && (
            <p style={{ fontFamily:FONT, fontSize:'0.8rem', lineHeight:1.6,
                        color:'rgba(102,192,244,0.5)', fontStyle:'italic', maxWidth:460,
                        borderLeft:'2px solid rgba(102,192,244,0.2)', paddingLeft:12 }}>
  ...
  return (
    <aside style={{ width:180, flexShrink:0 }} className="hidden lg:block sticky top-20 self-start">
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)',
        ...
        <div style={{ padding:'10px 14px 8px', borderBottom:'1px solid rgba(102,192,244,0.06)',
                      fontFamily:FONT, fontSize:'0.55rem', fontWeight:800,
                      letterSpacing:'0.18em', textTransform:'uppercase', color:'rgba(102,192,244,0.3)' }}>
          Discover
        </div>
        {QUICK.map(({ icon, label, sub, to, auth }) => {
          ...
          return (
            <button key={to} onClick={() => nav(to)} style={{
              ...
              border:'none', borderBottom:'1px solid rgba(102,192,244,0.04)',
            ...
            onMouseLeave={e => e.currentTarget.style.background='transparent'}>
              ...
              <div style={{ minWidth:0 }}>
                <div style={{ fontFamily:FONT, fontSize:'0.76rem', fontWeight:600,
                              color:'rgba(102,192,244,0.75)', whiteSpace:'nowrap',
                              overflow:'hidden', textOverflow:'ellipsis' }}>{label}</div>
                <div style={{ fontFamily:FONT, fontSize:'0.58rem', color:'rgba(102,192,244,0.28)' }}>{sub}</div>
              </div>
            </button>
          )
        })}
        ...
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)',
          ...
          {[['·','Diary','/diary'],['·','Library','/library'],['·','Dashboard','/dashboard']].map(([ico,lbl,to]) => (
            ...
            onMouseLeave={e => e.currentTarget.style.background='transparent'}>
              ...
              <span style={{ fontFamily:FONT, fontSize:'0.74rem', fontWeight:600, color:'rgba(102,192,244,0.6)' }}>{lbl}</span>
              <span style={{ marginLeft:'auto', fontSize:'0.65rem', color:'rgba(102,192,244,0.2)' }}>→</span>
            </button>
          ))}
        </div>
      )}
...
export default function Home() {
  useSEO({ title: 'Discover Games', description: 'Your game diary. Log, rate and review every game you play. Discover trending games and connect with other players.' })
  ...
  return (
    <div style={{ display:'flex', justifyContent:'center', alignItems:'flex-start', padding:'76px 0 60px' }}>
      ...
      <div style={{ flex:1, maxWidth:1140, minWidth:0, padding:'0 clamp(12px, 4vw, 20px)', display:'flex', gap:24, alignItems:'flex-start' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {/* Hero banner */}
          {(!user || !lastPlayed) && (
            <div style={{ borderRadius:20, border:'1px solid rgba(102,192,244,0.12)',
                          background:'linear-gradient(135deg, #111120 0%, #13131e 100%)',
                          padding:'clamp(20px, 5vw, 36px) clamp(20px, 5vw, 40px)', marginBottom:36, position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', top:0, right:0, width:280, height:280,
                            background:'radial-gradient(circle, rgba(102,192,244,0.06) 0%, transparent 70%)',
                            transform:'translate(30%,-30%)' }}/>
              {user ? (
                <>
                  <p style={{ fontFamily:FONT, fontSize:'0.6rem', fontWeight:800, letterSpacing:'0.2em',
                              textTransform:'uppercase', color:'rgba(102,192,244,0.4)', marginBottom:10 }}>Welcome back</p>
                  ...
                  <p style={{ fontFamily:FONT, fontSize:'0.85rem', color:'rgba(102,192,244,0.4)' }}>Log a game to see your Requiem here.</p>
                </>
              ) : (
                <>
                  <p style={{ fontFamily:FONT, fontSize:'0.6rem', fontWeight:800, letterSpacing:'0.2em',
                              textTransform:'uppercase', color:'rgba(102,192,244,0.35)', marginBottom:10 }}>A diary for the games you love</p>
                  <h1 style={{ fontFamily:FONT, fontWeight:900, fontSize:'clamp(1.6rem, 5vw, 2.4rem)', letterSpacing:'-0.03em',
                              color:TEAL, lineHeight:1.1, marginBottom:12 }}>Playquiem.</h1>
                  <p style={{ fontFamily:FONT, fontSize:'0.85rem', color:'rgba(102,192,244,0.45)', marginBottom:20, maxWidth:420 }}>
                    Track every game. Write your requiem. Share your journey.
                  </p>
                  <button onClick={() => nav('/auth')} style={{
                    background:TEAL, color:'var(--bg)', fontFamily:FONT,
              ...
              {tl
                ? Array.from({length:20}).map((_,i) => (
                    <div key={i} style={{ width:138, flexShrink:0, borderRadius:14, overflow:'hidden',
                                          background:'var(--surface)', border:'1px solid rgba(102,192,244,0.05)', scrollSnapAlign:'start' }}>
            ...
            <HScrollRow>
              {rl
                ? Array.from({length:8}).map((_,i) => (
                    <div key={i} style={{ flexShrink:0, scrollSnapAlign:'start', width:140 }}>
                      <SkeletonCard/>
                    </div>
                  ))
                : recent.map(g => (
                    <div key={g.id} style={{ flexShrink:0, scrollSnapAlign:'start', width:140 }}>
                      <GameCard game={g}/>
                    </div>
                  ))
              }
            ...
            {news.length === 0 ? (
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:14 }}>
                {Array.from({length:6}).map((_,i) => (
                  <div key={i} style={{ borderRadius:12, overflow:'hidden', background:'var(--surface)', border:'1px solid rgba(102,192,244,0.06)' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:14 }}>
                {news.map((a,i) => (
                  <a key={i} href={a.link} target="_blank" rel="noopener noreferrer"
                    style={{ display:'block', textDecoration:'none', background:'var(--surface)',
                              border:'1px solid rgba(102,192,244,0.07)', borderRadius:12, overflow:'hidden',
                              transition:'border-color 0.15s, transform 0.15s' }}
                    onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(102,192,244,0.22)';e.currentTarget.style.transform='translateY(-3px)'}}
                    onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(102,192,244,0.07)';e.currentTarget.style.transform='translateY(0)'}}>
                    ...
                    <div style={{ padding:14 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                        <span style={{ fontFamily:FONT, fontSize:'0.56rem', fontWeight:800, letterSpacing:'0.14em',
                                        padding:'2px 7px', borderRadius:4, background:'rgba(102,192,244,0.08)',
                                        border:'1px solid rgba(102,192,244,0.2)', color:TEAL }}>{a.source}</span>
                        <span style={{ fontSize:'0.58rem', color:'rgba(102,192,244,0.25)', fontFamily:FONT }}>{a.pubDate?.slice(5,16)}</span>
                      </div>
                      ...
                      {a.summary && (
                        <div style={{ fontFamily:FONT, fontSize:'0.7rem', lineHeight:1.55, color:'rgba(102,192,244,0.38)',
