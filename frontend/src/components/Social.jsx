...
function MiniAvatar({ name = '?', size = 36 }) {
  return (
    <div style={{
      ...
      fontWeight: 800, fontSize: size * 0.36, color: '#070b12',
    }}>
      {(name || '?')[0].toUpperCase()}
    </div>
  )
...
export function FriendButton({ targetId, size = 'md' }) {
  ...
  const CFG = {
    none:      { label:'+ Add Friend',   bg:TEAL,                     color:'#070b12', border:TEAL },
    sent:      { label:'Sent ✓',         bg:'transparent',            color:TEAL,      border:'rgba(14,165,233,0.35)' },
    received:  { label:'Accept',       bg:'rgba(14,165,233,0.15)', color:TEAL,      border:'rgba(14,165,233,0.5)' },
    following: { label:'Friends ✓',      bg:'transparent',            color:'rgba(14,165,233,0.45)', border:'rgba(14,165,233,0.15)' },
  }
  const c = CFG[status]
  return (
  ...
  return (
    <div
      ...
      style={{
        ...
        background: hov ? 'rgba(14,165,233,0.04)' : '#0f1c2e',
        border:`1px solid ${hov ? 'rgba(14,165,233,0.22)' : 'rgba(14,165,233,0.08)'}`,
    ...
    >
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.63rem', color:'rgba(14,165,233,0.35)', marginTop:2 }}>
...
export function WhoToFollow({ compact = false }) {
  ...
  return (
    <div style={{ background:'#0f1c2e', border:'1px solid rgba(14,165,233,0.08)', borderRadius:14, overflow:'hidden' }}>
      <div style={{ padding:'10px 14px 8px', borderBottom:'1px solid rgba(14,165,233,0.06)', fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.55rem', fontWeight:800, letterSpacing:'0.18em', textTransform:'uppercase', color:'rgba(14,165,233,0.3)' }}>
         Who to Follow
      </div>
      {loading ? <div style={{padding:16,display:'flex',justifyContent:'center'}}><Spinner size="sm"/></div>
       : suggestions.map(u => (
        <div key={u.id}
          onClick={e => { if (!e.target.closest('button')) nav(`/profile/${u.id}`) }}
          style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', borderBottom:'1px solid rgba(14,165,233,0.04)', cursor:'pointer', transition:'background 0.12s' }}
        ...
        >
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            {u.matchScore > 0 && <div style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.58rem', color:'rgba(14,165,233,0.28)' }}>{u.matchScore} shared genre{u.matchScore!==1?'s':''}</div>}
          </div>
...
export function PendingRequests() {
  ...
  return (
    <div style={{ background:'rgba(14,165,233,0.04)', border:'1px solid rgba(14,165,233,0.18)', borderRadius:14, overflow:'hidden', marginBottom:20 }}>
      <div style={{ padding:'11px 16px', borderBottom:'1px solid rgba(14,165,233,0.1)', fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.62rem', fontWeight:800, letterSpacing:'0.14em', textTransform:'uppercase', color:TEAL }}>
         Friend Requests ({requests.length})
      </div>
      {requests.map(req => (
        <div key={req.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', borderBottom:'1px solid rgba(14,165,233,0.05)' }}>
          ...
          <div style={{ flex:1, cursor:'pointer' }} onClick={() => nav(`/profile/${req.from_id}`)}>
            ...
            <div style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.63rem', color:'rgba(14,165,233,0.35)' }}>wants to be your friend</div>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={()=>handleAccept(req.from_id)} style={{ background:TEAL, color:'#070b12', fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontWeight:800, fontSize:'0.7rem', padding:'6px 14px', borderRadius:8, border:'none', cursor:'pointer' }}>Accept</button>
            <button onClick={()=>handleDecline(req.from_id)} style={{ background:'transparent', color:'rgba(14,165,233,0.4)', fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontWeight:700, fontSize:'0.7rem', padding:'6px 14px', borderRadius:8, border:'1px solid rgba(14,165,233,0.15)', cursor:'pointer' }}>Decline</button>
          </div>
        </div>
      ))}
    </div>
...
export function MemberSearch({ placeholder='Find players…', compact=false }) {
  ...
  return (
    <div ref={wrapRef} style={{ position:'relative' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, background:'rgba(14,165,233,0.06)', border:'1px solid rgba(14,165,233,0.15)', borderRadius:10, padding: compact?'7px 12px':'9px 14px' }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(14,165,233,0.4)" strokeWidth="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        ...
        {q && !loading && <button onClick={()=>{setQ('');setResults([]);setOpen(false)}} style={{background:'none',border:'none',color:'rgba(14,165,233,0.3)',cursor:'pointer',fontSize:16,lineHeight:1,padding:0}}>×</button>}
      </div>
      {open && results.length > 0 && (
        <div style={{ position:'absolute', top:'calc(100% + 6px)', left:0, right:0, background:'#0f1c2e', border:'1px solid rgba(14,165,233,0.15)', borderRadius:12, overflow:'hidden', zIndex:200, boxShadow:'0 16px 48px rgba(0,0,0,0.8)' }}>
          {results.map(u => (
            <div key={u.id} onClick={()=>{nav(`/profile/${u.id}`);setOpen(false);setQ('')}}
              style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', cursor:'pointer', borderBottom:'1px solid rgba(14,165,233,0.05)', transition:'background 0.12s' }}
            ...
            >
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.6rem', color:'rgba(14,165,233,0.3)' }}>{u.gameCount} games{u.isFollowing?' · Friends':''}</div>
              </div>
            </div>
          ))}
        </div>
...
export function MemberSearchPage() {
  ...
  const doSearch = useCallback(async val => {
    ...
    try { const d = await searchMembers(val, 20); setResults(d) }
    catch { setResults([]) }
  ...
  return (
    <div style={{ maxWidth:720, margin:'0 auto', padding:'76px 20px 60px' }}>
      ...
      <p style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.82rem', color:'rgba(14,165,233,0.4)', marginBottom:24 }}>Search by username · click to view their profile</p>

      <PendingRequests />

      {/* Search bar */}
      <div style={{ display:'flex', alignItems:'center', gap:10, background:'rgba(14,165,233,0.06)', border:'1px solid rgba(14,165,233,0.2)', borderRadius:14, padding:'13px 18px', marginBottom:28 }}>
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="rgba(14,165,233,0.4)" strokeWidth="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        ...
        {loading && <Spinner size="sm"/>}
        {q && !loading && <button onClick={()=>{setQ('');setResults([]);setSearched(false);clearTimeout(timerRef.current)}} style={{background:'none',border:'none',color:'rgba(14,165,233,0.3)',cursor:'pointer',fontSize:22,lineHeight:1,padding:0}}>×</button>}
      </div>
      ...
      {loading ? (
        ...
        <>
          <p style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.65rem', color:'rgba(14,165,233,0.3)', marginBottom:14, letterSpacing:'0.05em' }}>
        ...
        <div style={{ textAlign:'center', paddingTop:56 }}>
          ...
          <p style={{ fontFamily:'"Helvetica Neue",Helvetica,Arial,sans-serif', fontSize:'0.8rem', color:'rgba(14,165,233,0.35)' }}>Try a different username</p>
        </div>
      ) : (
        <WhoToFollow compact={false}/>
      )}
    </div>
  )
}
