-- ╔══════════════════════════════════════════════════════════╗
-- ║  Playquiem — Complete Migration (run this ONCE)         ║
-- ║  Paste into Supabase SQL Editor and click Run           ║
-- ╚══════════════════════════════════════════════════════════╝

-- ── 1. TAGS + REPLAYED on logs ────────────────────────────────
...
create table if not exists public.challenges (
  ...
  goal_value  integer default 52,
  goal_filter text default '',
...
create policy "Notifs delete own"              on public.notifications for delete  using  (auth.uid() = user_id);
...
begin
  ...
  if not exists (
    ...
    where user_id = new.following_id and actor_id = new.follower_id
      and type = 'follow' and created_at > now() - interval '10 seconds'
  ) then
...
-- New log → notify followers
create or replace function public.notify_followers_on_log()
returns trigger language plpgsql security definer as $$
declare actor_username text; follower record;
begin
  select username into actor_username from public.profiles where id = new.user_id;
  for follower in select follower_id from public.follows where following_id = new.user_id loop
    insert into public.notifications(user_id, type, actor_id, actor_name, message, link)
    values (
      follower.follower_id, 'log', new.user_id,
      coalesce(actor_username,'Someone'),
...
-- Request accepted → mutual follows + notify sender
...
-- Create avatars storage bucket (run this in Supabase Storage tab too)
insert into storage.buckets (id, name, public)
  values ('avatars', 'avatars', true)
  on conflict (id) do nothing;

-- Storage RLS: anyone can read, only owner can upload
